...
| Stream, keyterms, harness (Tasks 10–14) | DONE | Sep 16–18 |
| Session, server, front end (Tasks 15–17) | DONE | Sep 19–22 |
| Deploy (Task 18) | DONE | Render free tier, `docs/DEPLOY.md` |
...
- **2026-09-17** — Real-voice benchmark expanded 8 -> 26 items (day 22, time 3, service 1): the unused `ab_p2_t05` take plus 17 SLURP clips pulled individually through Hugging Face's dataset rows API (the 3.7GB Zenodo archive was abandoned at a measured 450KB/s). Sourcing them exposed three real defects, all fixed in `src/`, not worked around in the corpus. (1) `matcher.py` had no handling for digit-rendered hours: AssemblyAI's ITN returns "1 PM" while `TIMES` is spelled out, and `match("1 pm", TIMES)` ranked "two pm" (0.74) above the correct "one pm" (0.6467), past both thresholds, so the agent would have offered the wrong hour aloud. (2) The LLM binder returned no bindings at all for values stated plainly ("...Danielle's consultation...", "Clear my 9 AM alarms", "What's the weather next Wednesday?"); adding prompt examples fixed each target sentence and broke another, so `bind()` now falls back to an exact word-bounded scan of the field's own value set when the model binds nothing. It can only report a value already in the set, so it cannot invent or correct one, and a garbled value still has to come from the model. The prompt itself was reverted to its reviewed four-example form. (3) A bare reply to the question just asked ("It's Bennett", "a haircut please") bound nothing, because the binder had to infer the field from a one-word utterance with no sentence context; `session.py` now takes a reply that reduces to a single value from the asked field's set, under the same turn-confidence guard as `_commit_clean`. Suite 89 -> 97 tests. Caches for both real-voice sets were purged and both evals re-run so every number comes from one code version.
- **2026-09-17** — Real-voice results, 26 items. Clean: commit accuracy 96% baseline vs 100% Say Less, words re-said 0.77 vs 0.04, turns 1.15 vs 1.00, escalation 4% vs 0%. The whole gap is one item, `real_slurp_time_nineam`: the transcript's "9 AM" is out of set, and the baseline has no notion of a value set, so its confidence-only trigger fired repeatedly and the call escalated after 5 turns and 20 re-said words; Say Less matched the digit form to "nine am", offered it, and committed on a one-word yes. Noisy (same clips mixed with cafe babble, `scripts/generate_noisy_dataset.py`): both arms 96%, 0.92 words, 1.15 turns, 4% escalation -- identical, because the one item either arm loses is `real_slurp_sat2_noisy`, where noise replaced the day word entirely ("next saturday" -> "max out today"), which no set-membership check can recover. Honest reading: the mechanism earns its keep when noise or normalisation leaves a wrong-but-recoverable value, and is correctly powerless when the word is destroyed outright.
- **2026-09-17** — Deploy resolved: Render free tier, from the new `Dockerfile`. Vercel was tried first and rejected on evidence, not theory: it builds and serves the page, but its functions never delivered the agent's reply over the WebSocket (the server-side LLM Gateway call returned 200 in Vercel's own logs while the browser sat on "Reading back..."). Hugging Face Spaces cannot host it either -- Docker Spaces are paid-only on a free account and CPU Basic is not selectable -- and Koyeb has new services disabled. `docs/DEPLOY.md` records what was tried and what works. The server now runs the blocking binder call off the event loop so audio and the upstream keepalive keep flowing while it waits.
